...
export const booksWithGenresSelector = createSelector(
    [
        BooksModel.allBooksSelector,
        NavListModel.allGenresSelector,
       ],
    (books, genres) => {
        const booksByGenres: BooksByGenresTypes = [
            { genre: 'all', genreFilteredBooks: books }
        ];
        const countedGenres: CountedGenreType[] = [];

        genres.forEach(genre => {
            const genreFilteredBooks = books.filter(book =>
                book.categories.includes(genre.name));
