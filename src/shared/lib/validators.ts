...
export const validators = {
    ...
    loginValidator(fieldValue: string) {
        return ((
            this.digital(fieldValue, true) === true &&
            this.latinAlphabet(fieldValue) === true
    ...
    digital(fieldValue: string, isLogin: boolean) {
        return /\d+/.test(fieldValue) || (isLogin ?
            ErrorMessages.LOGIN_DIGITAL : ErrorMessages.PASSWORD_DIGITAL);
    },
    ...
    lengthLessThenEight(fieldValue: string) {
        return /^[\p{Alpha}\p{M}\p{Nd}\p{Pc}\p{Join_C}]{8,}$/gu.test(fieldValue) ||
    ...
    emailValidator(fieldValue: string) {
        return /^[\w%+.-]+@[\d.a-z-]+\.[a-z]{2,63}$/i.test(fieldValue) ||
